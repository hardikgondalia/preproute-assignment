const Login = () => {
  return (
    <div className="w-screen h-screen flex items-center justify-center bg-[#F7FBFF]">
      <div className="w-full h-full flex px-5 py-4.5">
        <div className="w-1/2 h-full flex justify-center items-center">
          <img src="/images/login-image.svg" alt="" className="lg:w-116.75 lg:h-86" />
        </div>
        <div className="w-1/2 h-full px-25 flex flex-col justify-center gap-7.5 bg-white text-[#374151] border border-[#60A5FA] rounded-lg">
          <img src="/images/logo.svg" alt="" className="w-33.75 h-8.5" />
          <div className="flex flex-col gap-5">
            <span className="text-[20px] font-semibold">Login</span>
            <span className="text-[12px] font-normal">Use your company provided Login credentials</span>
          </div>
          <form className="flex flex-col gap-7.5">
            <div className="flex flex-col gap-3.75 font-medium">
              <label htmlFor="userid">User ID</label>
              <input type="text" name="userid" placeholder="Enter User ID" className="py-2.75 px-4 border border-[#9CA3AF] rounded-lg outline-none" />
            </div>
            <div className="flex flex-col gap-3.75 font-medium">
              <label htmlFor="userid">Password</label>
              <input type="text" name="userid" placeholder="Enter Password" className="py-2.75 px-4 border border-[#9CA3AF] rounded-lg outline-none" />
            </div>
            <a href="" className="text-[#1B5DEF] text-[14px] cursor-pointer">Forgot password? </a>
          </form>
          <button className="border-none rounded-xl bg-[#5988Ef] text-[#FAFAFA] h-12 flex justify-center items-center cursor-pointer hover:bg-[#253183] transition-colors duration-300">Login</button>
        </div>
      </div>
    </div>
  );
};

export default Login;
